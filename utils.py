...
def read_for_OpenNE_from_mat(filename):
    
    G = og.Graph()
    
    print("Loading mat file for classification only")
    G.readMatFile(filename)
...
def split_train_test_graph(input_edgelist, seed, testing_ratio, weighted=False):
...
def split_train_test_graphReal(input_edgelist1, input_edgelist2, seed, testing_ratio, weighted=False):
    
    if (weighted):
        G1 = nx.read_weighted_edgelist(input_edgelist1)
        G2 = nx.read_weighted_edgelist(input_edgelist2)
    else:
        G1 = nx.read_edgelist(input_edgelist1)
        G2 = nx.read_edgelist(input_edgelist2)
    node_num1, edge_num1 = len(G1.nodes), len(G1.edges)
    node_num2, edge_num2 = len(G2.nodes), len(G2.edges)

    print('Original CoExp Graph: nodes:', node_num1, 'edges:', edge_num1)
    print('Original Exper Graph: nodes:', node_num2, 'edges:', edge_num2)
    testing_edges_num = int(len(G2.edges))
    random.seed(seed)
    testing_pos_edges = G2.edges
    G_train = copy.deepcopy(G1)
    overlapCount = 0
    for edge in testing_pos_edges:
        node_u, node_v = edge
        if(G_train.has_edge(node_u,node_v)):
            overlapCount = overlapCount+1
    print("Number of edge Overlap: ", overlapCount)        
...
def split_train_test_classify(embedding_look_up, X, Y, seed, testing_ratio=0.5):
